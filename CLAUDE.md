...
- **`ResSim`** (`__init__.py`) is a dataclass composed by multiple inheritance: `ResSim(NicePrint, Grid2D, Plot2D)`. OOP is used (rather than passing dicts) so ensemble forecasting (as in HistoryMatching) can hold independent instances whose parameters don't influence each other.
  ...
  - **Wells are one unified set** — there is no code-path distinction between injectors and producers. `well_rates` is **signed**: positive injects (water), negative produces (at the cell's fractional flow); the transport step discriminates by the sign of the assembled source field `_Q`, not by well identity. A BHP-controlled well's flow direction is likewise emergent (`realize_bhp`), and plot markers are inferred from the sign of the rate spec. `actual_rates`/`actual_bhp` are single `(nComp, nSteps)` arrays.
  - **A well may span several completions** (e.g. one per cell of a `well_path`), and it is the *completions* that the model assembles and indexes everything by. So `nComp` counts the rows of `well_xy`/`well_rates`/`actual_rates`, whereas `nWell` counts wells; `well_group` maps the former to the latter, and `well_names` names them. The grouping serves the *reporting* alone (`rates_by_well`, the plot labels) — the physics never groups, and `well_group` may be left `None`, meaning one well per completion.
  - **Wells are configured by assigning `wells`** (or `ResSim(wells=[...])`): one record (`dict`) per well — `xy` or `path`, `rate` and/or `bhp`, `rw`/`skin` or `WI`, `name` — from which `__setattr__` (via the private `_set_wells`) assembles the flat, per-completion arrays, the grouping included. Those arrays are the model's state and remain assignable by hand, which HistoryMatching relies on; conversely, assigning one of them resets `wells` to `None`, so the records never describe a superseded config.
  - **`__setattr__` does normalization magic**: assigning `well_xy` snaps well positions to the nearest grid node; `well_rates`/`well_bhp` are reshaped to `(nComp, nTime)`; `well_WI` is broadcast to `(nComp,)`; `well_group` to an int `(nComp,)`; assigning `wells` assembles all of those (`_set_wells`); scalar `K` is broadcast to shape `(2, Nx, Ny)`, and `K`/`por` get their grid-shaped defaults here when left `None` (which is why there is no `__post_init__`). It is the single normalization layer — `_set_wells` routes its assignments through it rather than writing past it. When `ct == 0` (and no BHP-controlled well), the rates must sum to zero at every time index (asserted in `time_stepper` — otherwise mass imbalance would silently leak in the SW corner). With `ct > 0` the imbalance is absorbed by storage (enabling e.g. primary depletion by a lone producer).
